...
from msda_confidenceloss.rpn import _RPN
...
from msda_confidenceloss.utils import get_max_iou, consist_loss
    ...
    def forward(
        ...
        if subnet == "subnet1":
            ...
            if self.training:
                ...
                rpn_bbox_pred1, scores1 = self.RCNN_rpn1(
                    base_feat_1, im_info, gt_boxes, num_boxes, ema=True
                )
                rpn_bbox_pred2, scores2 = self.RCNN_rpn2(
                    base_feat_2, im_info, gt_boxes, num_boxes, ema=True
                )
                rpn_bbox_pred_ema, scores_ema = self.RCNN_rpn_ema(
                    base_feat, im_info, gt_boxes, num_boxes, ema=True
                )

                consist_loss1 = consist_loss(rpn_bbox_pred1, scores1, rpn_bbox_pred_ema, scores_ema)
                consist_loss2 = consist_loss(rpn_bbox_pred2, scores2, rpn_bbox_pred_ema, scores_ema)
