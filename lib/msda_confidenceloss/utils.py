...
def consist_loss(pred, scores, pred_ema, score_ema):
    loss = 0
    pred = pred[0, :256, :]
    scores = scores[0, :256]
    pred_ema = pred_ema[0, :256, :]
    scores_ema = score_ema[0, :256]
    count = 0
    for idx, gt in enumerate(pred_ema):
        ious, max_iou, max_index = get_max_iou(pred,gt)
        # loss += abs(max_index - idx) * max_iou
        if (scores[max_index] - scores_ema[idx]) * max_iou >0.0001 :
            loss += abs(scores[max_index] - scores_ema[idx]) * max_iou
            count +=1
    if count > 1:
        loss /= count
    return loss
