...
class _fasterRCNN(nn.Module):
    ...
    def forward(
            ...
            subnet1_base_feat2=None, subnet2_base_feat2=None, have_base_feat2=True, tsne=False
    ):
        ...
        if subnet == "subnet1":
            ...
            #
            # try:
            #     if self.RCNN_base_sub1[14].weight.grad:
            #         pass
            # except RuntimeError:
            #     tmp = base_feat1.detach()
            #     tmp_RCNN_base_sub1 = copy.deepcopy(self.RCNN_base_sub1)
            #
            #     for i in range(14):
            #         tmp = tmp_RCNN_base_sub1[i](tmp)
            #
            #     myconv = copy.deepcopy(self.RCNN_base_sub1[14])
            #     myReLU = copy.deepcopy(self.RCNN_base_sub1[15])
            #     myconv.weight = torch.nn.Parameter(self.RCNN_base_sub1[14].weight.grad)
            #     tmp = myconv(tmp)
            #     tmp = myReLU(tmp)
            #     tmp = torch.sum(tmp[0], 0).cpu().detach().numpy()
            #     # tmp = tmp.permute(1, 2, 0).cpu()
            #     plt.imshow(tmp)
            #     plt.show()
            ...
            # #image
            # plt.imshow(torch.sum(im_data[0], 0).cpu().detach().numpy())
            # plt.show()
            #
            # #base feature
            # plt.imshow(torch.sum(base_feat1[0], 0).cpu().detach().numpy())
            # plt.show()
            ...
            if self.training:
                ...
                # # source 1 feature
                # plt.imshow(torch.sum(base_feat_1[0], 0).cpu().detach().numpy())
                # plt.show()
                #
                # # source 2 feature
                # plt.imshow(torch.sum(base_feat_2[0], 0).cpu().detach().numpy())
                # plt.show()
                #
                # # ema feature
                # plt.imshow(torch.sum(base_feat[0], 0).cpu().detach().numpy())
                # plt.show()
